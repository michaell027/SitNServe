...
function RestaurantInfoScreen({ navigation, route }) {
  ...
  const renderOpeningHours = () => {
    ...
    return daysOfWeek.map((day) => (
      <View key={day} className={'flex flex-row justify-between py-1 sm:border-b-[1px] sm:mb-4'}>
      ...
      <View className={'min-h-[90vh]'}>
        ...
        {content == 'openingHours' && (
          <View className={'mt-4 px-2'}>
            <Text className={'text-black text-xl font-bold mb-2 sm:mb-6'}>Opening Hours</Text>
            {renderOpeningHours()}
          </View>
        )}
        {content == 'map' && (
